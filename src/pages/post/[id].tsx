        ...
        <main className="flex items-center justify-start min-h-[calc(100vh-70px)] flex-col">
            <div className="flex items-start justify-center w-[90%] max-w-[1000px] z-[0] flex-col">
                ...
                <div className="flex flex-row mb-[20px]">
                    {post.tags.map((tag) => (
                        <article className="bg-[#030303] mr-[10px] text-[12px] py-[6px] px-[10px] duration-[0.3s] select-none border border-input border-[1px] hover:bg-[#1c1c1c] rounded-[8px]">{tag}</article>
                    ))}
                </div>
                {post.imgs.map((item) => (
                    <img src={item} alt={item} className="rounded-[10px] mb-[20px] h-[400px] md:h-[450px] w-full"/>
                ))}
            </div>
        </main>
    )
